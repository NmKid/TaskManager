...
*   **Calendar → Tasks (カレンダーからタスクへ)**
    *   **新規予定:** カレンダーに新しい予定が作成されると、対応するタスクをGoogle Tasksに自動作成する。
...
## 5. 運用フロー
1.  ユーザーがGoogle Tasksにタスクを登録（またはCalendarに予定登録）。
2.  スクリプト（`main.py`）を実行。
3.  Calendar → Tasks の同期実行。
4.  Tasks の未処理タスクを分析・分割。
5.  Calendar の空き枠にタスク・移動予定を登録。
6.  結果確認（タスク名の変更、カレンダーへの反映）。
